...
在控制器的方法中，直接执行 '$this->display()' 操作，会展现模板目录下的： 当前主题/控制器名称/方法名称.html

###数据读取的链式操作

支持 M()->field()->where()->limit()->find()  这样类似的操作

###安全的数据删除操作

在执行delete操作时，如果使用的是链式的API，那么当没有设置where条件时，会提示不合法的操作。
...
本地文件读取的路径默认相对于 PATH_APP 网站的根目录
